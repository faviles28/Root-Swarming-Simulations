...
to setup-patches
  ask patches [
    if pycor < (max-pycor / 5) [
      ...
      set ground-water ground-water + moisture
    ]
...
to go
  ...
  set-moisture-orientation
  set-forces-orientation
  set-velocity
  root-growth
  absorb-moisture
  natural-branch
...
to set-forces-orientation
    ask turtles [
    let nearby-turtles turtles in-radius radius with [root-id != [root-id] of myself]
    ifelse any? nearby-turtles [
      let orientation-sum sum [orientation] of nearby-turtles
      let num-nearby count nearby-turtles
      let average-orientation orientation-sum / num-nearby
      let fluctuation (random-float 1 - 0.5) * 20
      set orientation average-orientation + fluctuation
    ]
    [
      let fluctuation (random-float 1 - 0.5) * 10
      set orientation orientation + fluctuation
    ...
    let total-forces-x 0
    let total-forces-y 0
    ask nearby-turtles [
      let other-turtle self
      let r distance self
      let diff-xcor xcor - [xcor] of myself
      let diff-ycor ycor - [xcor] of myself
      let magnitude sqrt (diff-xcor * diff-xcor + diff-ycor * diff-ycor)
      if magnitude > 0 [
        let v1 diff-xcor / magnitude
        let v2 diff-ycor / magnitude
        let force-a-x 0.6 * (exp (0 - (0.09 * r * r))) * v1
        let force-a-y 0.6 *(exp (0 - (0.09 * r * r))) * v2
        let force-r-x 0 - (0.6 * (exp (0 - (0.1 * r * r))) * v1)
        let force-r-y 0 - (0.6 * (exp (0 - (0.1 * r * r))) * v2)
        set total-forces-x total-forces-x + force-a-x + force-r-x
        set total-forces-y total-forces-y + force-a-y + force-r-y
      ]
    ]

    if total-forces-x > 0 and total-forces-y > 0 [
      let force-angle-rad atan total-forces-y total-forces-x
      let force-angle-deg force-angle-rad * 180 / pi
      let new-orientation orientation + force-angle-deg
      set orientation new-orientation / 2
    ]
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot (100 * ground-water-used / ground-water)"
